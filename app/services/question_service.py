...
from app.schemas.question_schema import QuestionCreate, QuestionUpdate, QuestionUpdateByReponse
...
class QuestionService:
    ...
    async def update_question(current_user: User, question_id: UUID, data: QuestionUpdate):
        ...
        await question.save()
        return question
    # update question by reponse
    @staticmethod
    async def update_question_by_reponse(current_user: User, question_id: UUID, data: QuestionUpdateByReponse):
        question = await QuestionService.retrieve_question(current_user, question_id)
        await question.update({"$set" : data.dict(exclude_unset=True)})
        await question.save()
        return question
